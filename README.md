...
| **引擎 CJK patch** | ✅ **動詞列 + 句子列 + 多行對白實機渲染中文** |
| 對白翻譯工作流 | ✅ runtime 攔截真實字串 → 翻 → 免重編即生效 |
| 完整對白翻譯 | 🛠️ 機制全通,逐句翻譯進行中(~5400 句) |
...
接著最關鍵的一步——**讓印第開口講中文**。瓊斯一到紐約,內心獨白自動跳出來,三行斷得好好的:

![FOA 中文對白](screenshots/foa_cht_dialogue.png)

> 「好啦,我人是到紐約了。真搞不懂蘇菲亞·哈普古德怎麼會跟納粹間諜攪和到一塊去。」
> 這不是字面直譯——是**印第的口吻**:那股「都火燒眉毛了還在碎念」的乾。多行對白會自動斷行。
> 能可靠翻到這句,靠的是 **runtime 攔截**:patch 讓引擎把每句畫出來的英文原文 log 出來,
> 翻的是遊戲真正會顯示的字串,不是離線猜的——一翻就中,不必逐句試。
