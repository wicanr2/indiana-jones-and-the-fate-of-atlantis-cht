...
- [ ] **offset 攔截 + 重導**:patch `startTalkSound`——dump 每句的 offset(對應已知台詞)、命中重導表就改播外部中文 wav。
- [ ] 小段試播驗證(headless 無音效裝置 → 確認 hook 觸發 + stream 建立;聲音由使用者實聽)。
- [ ] 嘴型同步(mouth sync)沿用原 timing 或重算;音長對齊。
- [ ] 配音分角色:印第=男聲、蘇菲亞=女聲(`zh-CN-XiaoxiaoNeural`)、納粹=冷硬男聲。
