...
**待辦(Phase 3 收尾)**:
- [x] **大字對白(rule 81 不縮字)**:`displayDialog` 用 RAII guard 換 24×24 字型(`atlantis_zh24.dcjk` 放 game 夾),動詞 UI 保持 12×12;`_textSurfaceMultiplier=2` 在 DOS 版會破圖故不走。`build_translation.py` 把對白自動斷行(~11 CJK 字/行,配 24px 寬度),不再靠手填 \n,不丟字。實機驗證(開場旁白 5 行大字)。
  - [ ] 斷行避免單字孤行(如「哈普古德」被切成「哈普古/德」)——小修。
...
**台語版 = 獨立 milestone(暫緩)**。可行性已測:Meta MMS `nan` 能出台語音,但**吃白話字/台羅拼音(POJ),不吃漢字**(vocab 49 token 全是拉丁字母+調符;餵漢字直接失敗)。樣本 `voice_test/mms/mms_{roman,poj}.wav`。
真‧台語版要:① 把遊戲翻成**台語白話文**(獨立翻譯)② 漢羅轉 POJ ③ MMS 合成。**決定(使用者)**:先完成中文(國語)翻譯,台語當後續 milestone。
