...
**⚠️ 批次配音的真正瓶頸 = offset 來源**:
- talkie offset 走訊息內 `\xFF\x0A`+14 byte(`string.cpp:509`),但**不直接存在 `.001`**(經 script bytecode 組出,搜不到 byte pattern);靜態抽取要寫 descumm 級反組譯器。
- 可靠路徑 = **runtime 攔截**(已驗證準:dump 出 `CHTVOICE<offset>` ↔ `CHTMISS<text>` 配對),但**覆蓋率受限**(要實際玩到該句),非一鍵全包。
- [ ] 短期:harvest 開場可達場景的 offset → 批次配那批已翻句。
- [ ] 長期:寫 SCUMM v5 script 反組譯器靜態抽 (text, offset) 全集,才能真正一鍵批次。
...
- [ ] **F8 切文字語言**:英文原版 / 中文(DisplayMode toggle,charset 攔截開關)。
- [ ] **F9 切語音**:英文原音 / 中文國語 / (台語)——voice redirect 開關 + 切換音檔集。
- [ ] 鍵綁在 scumm 引擎 input handler;狀態存 ConfMan。
